...
import com.otoomo.codegenerator.core.*;
...
public class XGenCollectionParser1 implements IGenCollectionParser {
	...
	private void parseQueryConditions(Field field, XGenField xf, List<Map<String, Object>> qcs) {
		if (xf.query()) {
			String type = xf.componentType().getValue();
	 ...
	 * @param xf
	 * @param columns <p>
	 *                添加前端组件类型字段:componentType
	 * @updateBy modongning
	 * @updateDate 2019/10/16 下午4:14
	 * </p>
	 */
	private void parseColumns(Field field, XGenField xf, List<Map<String, Object>> columns) {
		...
		ComponentType componentType = xf.componentType();
		EnumData[] enumData = xf.enumData();

		column.put("fieldName", fieldName);
		column.put("columnName", GenerateUtil.toDbName(fieldName));
		column.put("length", length);
		column.put("componentType", componentType.getValue());
		column.put("type", field.getGenericType().toString());
		column.put("typeName", field.getType().getSimpleName());

		List<Map<String, String>> enumItemList = new ArrayList<>();
		StringBuilder enumDataStr = new StringBuilder();
		for (EnumData enumItem : enumData) {
			Map<String, String> enumItemMap = new HashMap<>();
			enumItemMap.put("key", enumItem.key());
			enumItemMap.put("value", enumItem.value());
			enumItemList.add(enumItemMap);

			enumDataStr.append(",").append(enumItem.key()).append(":").append(enumItem.value());
		}
		column.put("comment", xf.comment()+enumDataStr.toString());
		column.put("enumData", enumItemList);
